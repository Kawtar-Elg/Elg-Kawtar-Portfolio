...
export default function Index() {
  ...
  return (
    <WorkspaceShell>
      <WorkspaceHeader path="README.md" action={<><Link to="/repositories" className="workspace-header-search">⌕ Find a repository... <kbd>/</kbd></Link><WorkspaceActionButton asChild><a href="#collaboration">Open a collaboration</a></WorkspaceActionButton></>} />
      <WorkspaceTabs />
      <div className="workspace-overview-stack">
        ...
        <section id="repositories" aria-labelledby="pinned-title" className="workspace-section">
          ...
          <div className="workspace-pinned-grid">{featured.map((repository, index) => <RepositoryCard key={repository.id} repository={repository} layout={index === 0 ? "featured" : index === 1 ? "featured" : "compact"} onWatchDemo={setVideoRepository} onOpenLiveSite={repository.title === "Robotics Club CMC" ? setLiveRepository : undefined} />)}</div>
        </section>
