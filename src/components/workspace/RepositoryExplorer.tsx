  ...
  return (
    <>
      <section className="workspace-explorer" aria-labelledby="repositories-title">
        ...
        <div className="workspace-explorer-layout"><div className="workspace-repository-list">{filteredRepositories.length ? filteredRepositories.map((repository, index) => <RepositoryCard key={repository.id} repository={repository} layout={index % 3 === 0 ? "featured" : "list"} onWatchDemo={setVideoRepository} onOpenLiveSite={repository.title === "Robotics Club CMC" ? setLiveRepository : undefined} />) : <div className="workspace-empty-state"><span>⌘</span><p>No repositories match this filter.</p><button type="button" onClick={clearFilters}>Clear filters</button></div>}</div><aside className="workspace-explorer-sidebar"><div className="workspace-panel workspace-repo-map"><p className="workspace-code-label">repository map</p>{["Mobile", "Web", "Other"].map((item) => <div key={item}><span>{item}</span><strong>{repositories.filter((repository) => repository.category === item).length}</strong></div>)}<div><span>Primary language</span><strong className="workspace-text-green">Kotlin</strong></div></div><div className="workspace-panel"><p className="workspace-code-label">browse by topic</p><div className="workspace-topic-row">{allTopics.slice(0, 9).map((item) => <button type="button" key={item} className="workspace-topic" onClick={() => setTopic(item)}>{item}</button>)}</div></div><div className="workspace-panel workspace-explorer-note"><p><span className="workspace-text-green">ⓘ</span> Project popularity metrics are intentionally omitted. Browse the work through its real descriptions, technologies, screens, and links.</p></div></aside></div>
      </section>
